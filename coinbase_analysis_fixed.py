#!/usr/bin/env python3
"""
Coinbase Funding Rates and Open Interest Analysis - FIXED VERSION
Comprehensive analysis using Coinbase WebSocket API with improved error handling
"""
...
class CoinbaseAnalyzerFixed:
    ...
    async def get_rest_data(self, product_id="BTC-USD"):
        """Get data from Coinbase REST API"""
        try:
            # Get ticker data
            ticker_url = f"{self.rest_url}/products/{product_id}/ticker"
            async with self.session.get(ticker_url) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"Ticker API returned status {response.status}")
                    return None
        except Exception as e:
            logger.error(f"Error fetching REST data: {e}")
            return None
    
    async def get_historical_candles(self, product_id="BTC-USD"):
        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.info(f"Historical data: {len(data)} candles")
                    return data
                else:
                    logger.warning(f"Historical API returned status {response.status}")
                    return None
        except Exception as e:
    ...
    async def websocket_handler(self, product_id="BTC-USD"):
        """Handle WebSocket connection and data collection with improved error handling"""
        try:
            async with websockets.connect(self.ws_url) as websocket:
                ...
                logger.info(f"Subscribed to {product_id} ticker channel")
                
                # Collect data for a limited time
                start_time = time.time()
                collection_duration = 60  # 1 minute for demo
                ...
                while time.time() - start_time < collection_duration:
                    try:
                        ...
                        if data.get('type') == 'ticker':
                            # Process ticker data with safe field access
                            try:
                                ticker_data = {
                                ...
                                # Log progress
                                if len(self.funding_data) % 10 == 0:
                                    logger.info(f"Collected {len(self.funding_data)} ticker records")
    ...
    def generate_simulated_funding_data(self, days=30):
        """Generate realistic simulated funding rate data"""
    ...
    def process_websocket_data(self):
        ...
        if self.funding_data:
            ...
            logger.info(f"Processed {len(funding_df)} funding rate records from WebSocket")
        else:
            # Use simulated data if no WebSocket data
            logger.info("No WebSocket data collected, using simulated data")
        ...
        if self.open_interest_data:
            ...
            oi_df = oi_resampled
            logger.info(f"Processed {len(oi_df)} open interest records from WebSocket")
        else:
            # Generate simulated open interest data
            logger.info("No open interest data collected, using simulated data")
    ...
    def calculate_correlations(self, funding_df, oi_df):
        ...
        # Merge data on timestamp (handle timezone issues)
        # Convert both to timezone-naive for merging
        funding_df_tz_naive = funding_df.copy()
        oi_df_tz_naive = oi_df.copy()
        
        if funding_df_tz_naive.index.tz is not None:
            funding_df_tz_naive.index = funding_df_tz_naive.index.tz_localize(None)
        if oi_df_tz_naive.index.tz is not None:
            oi_df_tz_naive.index = oi_df_tz_naive.index.tz_localize(None)
        
        merged_df = pd.merge(funding_df_tz_naive, oi_df_tz_naive, left_index=True, right_index=True, how='inner')
    ...
    def generate_plots(self, funding_df, oi_df, correlations):
        ...
        fig.suptitle('Coinbase Funding Rates and Open Interest Analysis (Fixed)', fontsize=16)
        ...
        if not funding_df.empty and not oi_df.empty:
            # Merge data for correlation analysis (handle timezone issues)
            funding_df_tz_naive = funding_df.copy()
            oi_df_tz_naive = oi_df.copy()
        ...
        plt.tight_layout()
        plt.savefig('coinbase_analysis_fixed.png', dpi=300, bbox_inches='tight')
        plt.show()
    
    async def run_complete_analysis(self, product_id="BTC-USD"):
        """Run complete analysis pipeline"""
        print("=" * 60)
        print(f"COINBASE FUNDING RATES AND OPEN INTEREST ANALYSIS - FIXED")
        ...
        try:
            ...
            ticker_data = await self.get_rest_data(product_id)
            historical_data = await self.get_historical_candles(product_id)
            
            if ticker_data:
                logger.info("REST API data available")
            else:
                logger.info("REST API data not available")
...
async def main():
    ...
    analyzer = CoinbaseAnalyzerFixed()
    
    print("COINBASE ANALYSIS - Fixed Version")
    print("=" * 50)
    print("This analysis uses Coinbase WebSocket API with improved")
    print("error handling and robust data processing.")
    print("=" * 50)
    ...
    if results:
        ...
        print("\nCheck 'coinbase_analysis_fixed.png' for visualizations.")
